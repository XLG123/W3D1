...
def substrings(string)
  ...
  string.each_char.with_index do |char, idx|
    (idx..string.length-1).each do |i|
      sub_strs << "#{string[idx..i]}"
    end
  end
...
def subwords(word, dictionary)
  all_strs = substrings(word)
  dictionary.select { |dict_word| all_strs.include?(dict_word) }
end
...
def concatenate(strings)
  strings.inject { |phrases, phrase| phrases << " #{phrase}" }
end